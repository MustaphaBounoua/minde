...
class VariancePreservingTruncatedSampling:
    ...
    def Phi(self, t, T):
        ...
        mask_le_t_eps = (t <= self.t_epsilon).float()
        phi = mask_le_t_eps * \
            self.phi_t_le_t_eps(t) + (1. - mask_le_t_eps) * \
            self.phi_t_gt_t_eps(t_new)
        return phi / Z
...
def sample_vp_truncated_q(shape, beta_min, beta_max, t_epsilon, T):
    ...
    vpsde = VariancePreservingTruncatedSampling(
        beta_min=0.1, beta_max=20., t_epsilon=t_epsilon)
...
def ve_truncated_q_inv_Phi(u, sigma_min, sigma_max, t_epsilon, T):
    ...
    # nonlinear
    x_n = torch.log((r ** t_epsilon - 1) *
                    torch.exp((A * u - A1)) + 1) / np.log(r)
...
def ve_truncated_q_Phi(t, sigma_min, sigma_max, t_epsilon, T):
    ...
    # nonlinear
    u_n = A1 / A + (torch.log(m * r ** t - m) -
                    np.log(m * r ** t_epsilon - m)) / A
