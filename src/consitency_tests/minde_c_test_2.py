...
class Minde_mnist_c(pl.LightningModule):
    ...
    def log_samples(self):
       ...
       # cond_in_0=  self.destanderdize(deconcat(x1,mod_list=["x1","x2","y","hy"],sizes= [self.dim_x,self.dim_x,self.dim_y,self.dim_y]  ) )
        cond_in_1=  self.destanderdize(deconcat(x2,mod_list=["x1","x2","y","hy"],sizes= [self.dim_x,self.dim_x,self.dim_y,self.dim_y] ) )
     ...
     #   cond_in_out_1 =self.decode(cond_in_0)
      ...
      #  cond_samp_0=  self.destanderdize(deconcat(output_cond_0,mod_list=["x1","x2","y","hy"],sizes= [self.dim_x,self.dim_x,self.dim_y,self.dim_y]) )
        cond_samp_1=  self.destanderdize(deconcat(output_cond_1,mod_list=["x1","x2","y","hy"],sizes= [self.dim_x,self.dim_x,self.dim_y,self.dim_y]) )
...
if __name__ =="__main__":
    ...
    CHECKPOINT_DIR = "runs/trained_models/mld_c_mi_datapross_64/"+str(args.seed)+"/"

    tb_logger = TensorBoardLogger(save_dir=CHECKPOINT_DIR,
                                    name="t1_"+str(rows)
                                    )
    
    #mld.stat = get_stat("/home/*****/work/mi/runs/trained_models/mld_c_mi_datapross_64/1/t1_5/version_0/stat.pickle")
    trainer = pl.Trainer(
                logger=tb_logger,
    ...
    trainer.fit(model=mld, train_dataloaders=train_l,
        ...
        #                ckpt_path = "/home/*****/work/mi/runs/trained_models/mld_c_mi_datapross_64/1/t1_5/version_0/checkpoints/epoch=599-step=562200.ckpt/"
                   #     ckpt_path  = "/home/*****/work/mi/runs/trained_models/mld_c_mi_datapross/3/t1_5/version_0/checkpoints/epoch=599-step=140400 copy.ckpt"
