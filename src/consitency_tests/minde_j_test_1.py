...
class Minde_MLD(pl.LightningModule):
    ...
    def __init__(self,dim_x,dim_y ,lr = 1e-3,mod_list=["x","y"],use_skip = True, 
        ...
        if use_skip == True:
            ...
            hidden_dim=256
            time_dim = hidden_dim
            self.score = UnetMLP(dim= (dim_x +dim_y) , init_dim= hidden_dim ,dim_mults=(1,1), time_dim= time_dim ,nb_mod= 2 )
    ...
    def log_samples(self):
        ...
        log_modalities(self.logger, test_samp_small, ["x","y"], self.current_epoch ,prefix="real/" ,nb_samples=8)

        z_c = self.encode(test_samp_small)

        decode_ae = self.decode(z_c)

        log_modalities(self.logger, decode_ae, ["x","y"], self.current_epoch ,prefix="real_ae/" ,nb_samples=8)


        z_c = self.standerdize(z_c)
        z_c = concat_vect(z_c)
        masks = [
             self.get_mask([self.dim_x,self.dim_y],subset=[0],shape= z.shape).to(self.device),
             self.get_mask([self.dim_x,self.dim_y],subset=[1],shape= z.shape).to(self.device)
        ]
        ...
        log_modalities(self.logger, cond_in_out_2, ["x","y"], self.current_epoch ,prefix="cond_1_in/" ,nb_samples=8)
        ...
        log_modalities(self.logger, output_cond_1_im, ["x","y"], self.current_epoch ,prefix="cond_1/" ,nb_samples=8)
...
if __name__ =="__main__":
    ...
    CHECKPOINT_DIR = "runs/trained_models/mld_mi_row/"+str(args.seed)+"/"

    tb_logger = TensorBoardLogger(save_dir=CHECKPOINT_DIR,
                                    name="crop_"+str(rows)
                                    )
    
    mld.stat = get_stat("/home/*****/work/mi/runs/trained_models/mld_mi_row/2/crop_2/version_0/stat.pickle")
    trainer = pl.Trainer(
                logger=tb_logger,
    ...
    trainer.fit(model=mld, train_dataloaders=train_l,
               ...
               ckpt_path  ="/home/*****/work/mi/runs/trained_models/mld_mi_row/2/crop_2/version_0/checkpoints/epoch=249-step=234250.ckpt" 
                          )