...
class Minde_mnist_c(pl.LightningModule):
    ...
    def __init__(self,dim_x,dim_y ,lr = 1e-3,var_list=["x","y"],use_skip = True, 
        ...
        if use_skip == True:
            ...
            hidden_dim=256
            time_dim = hidden_dim
            self.score = UnetMLP(dim= (dim_x +dim_y) *2, init_dim= hidden_dim ,dim_mults=(1,1), time_dim= time_dim ,nb_mod= 2,out_dim = (dim_x +dim_y) )
    ...
    def get_mod_cropped_dataset(self,test_l):
        X1 = torch.Tensor().to(self.device)
        X2 = torch.Tensor().to(self.device)
        Y1 = torch.Tensor().to(self.device)
        Y2 = torch.Tensor().to(self.device)
        ...
        for batch in test_l:
            ...
            y1 = x1.clone().to(self.device)
            y2 = x2.clone().to(self.device)
    ...
    def get_mod_cropped(self,x1,x2):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.train()
        data =self.get_mod_cropped(batch["x1"],batch["x2"])
        z = self.encode(data)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        batch =self.get_mod_cropped(batch["x1"],batch["x2"])
        z = self.encode(batch)
        z =self.standerdize(z)
        ...
        loss = self.sde.train_step_cond(z,self.score,d = self.d).mean()  # # forward and compute loss
    ...
    def log_samples(self):
        ...
        x_c = self.standerdize(deconcat(x_c,["x1","x2","y1","y2"],sizes= [self.dim_x,self.dim_x,self.dim_y,self.dim_y]) )
        ...
        z_samp = self.destanderdize(deconcat(z,["x1","x2","y1","y2"],sizes= [self.dim_x,self.dim_x,self.dim_y,self.dim_y]))

        output = self.decode(z_samp)

        log_modalities(self.logger, output, ["x1","x2","y1","y2"], self.current_epoch ,prefix="sampling/" ,nb_samples=8)
        ...
        test_samp_small = {
             ...
             "y1": self.test_samples["y1"][:8].to(self.device),
             "y2": self.test_samples["y2"][:8].to(self.device)
        }

        log_modalities(self.logger, test_samp_small, ["x1","x2","y1","y2"], self.current_epoch ,prefix="real/" ,nb_samples=8)
    ...
    #    log_modalities(self.logger, cond_in_out_1, ["x1","x2","y1","y2"], self.current_epoch ,prefix="cond_0_in/" ,nb_samples=8)
        log_modalities(self.logger, cond_in_out_2, ["x1","x2","y1","y2"], self.current_epoch ,prefix="cond_1_in/" ,nb_samples=8)

        if self.use_ema:
       #     output_cond_0 = self.sde.modality_inpainting_c(score_net=self.model_ema.module,x = x1 , mask = masks[0],  subset=[0])
            output_cond_1 = self.sde.modality_inpainting_c(score_net=self.model_ema.module,x = x2 , mask = masks[1], subset=[1])
        else:
      #      output_cond_0 = self.sde.modality_inpainting_c(score_net=self.score,x = x1 , mask = masks[0],  subset=[0])
    ...
    #    output_cond_0_im = self.decode(cond_samp_0)
     ...
     #   log_modalities(self.logger, output_cond_0_im, ["x1","x2","y1","y2"], self.current_epoch ,prefix="cond_0/" ,nb_samples=8)
        log_modalities(self.logger, output_cond_1_im, ["x1","x2","y1","y2"], self.current_epoch ,prefix="cond_1/" ,nb_samples=8)
    ...
    def log_mi(self):
            ...
            self.test_samples["y1"]=self.test_samples["y1"].to(self.device)
            self.test_samples["y2"]=self.test_samples["y2"].to(self.device)
...
if __name__ =="__main__":
    ...
    train_l, test_l = get_mnist_dataset(batch_size= 1000,drop_last=False)
    train_l_2, test_l_2 = get_mnist_dataset(batch_size= 1000,drop_last=False)
    ...
    for batch in train_l:
        X1_train = torch.cat([X1_train,batch[0]])

    for batch in train_l_2:
        X2_train = torch.cat([X2_train,batch[0]]) 
    
    for batch in test_l:
        X1_test = torch.cat([X1_test,batch[0]])

    for batch in test_l_2:
        X2_test = torch.cat([X2_test,batch[0]]) 

    
    train = MNIST_pairs(X1_train,X2_train)
    test = MNIST_pairs(X1_test,X2_test)

    print(len(train))
    print(len(test))

    train_l_p = DataLoader(train, batch_size=Batch_size,
                          shuffle= True,
                          num_workers=2, drop_last=True,pin_memory =True)
    
    test_l_p = DataLoader(test, batch_size=Batch_size,
                          shuffle= False,
    ...
    CHECKPOINT_DIR = "runs/trained_models/mld_c_mi_addit/"+str(args.seed)+"/"
    ...
    trainer.fit(model=mld,train_dataloaders=train_l_p,val_dataloaders=test_l_p)